...
const seedAdmin = async () => {
  try {
    ...
    const adminExists = await User.findOne({ email: 'admin@shringaar.com' });
    if (!adminExists) {
      await User.create({
        name: 'Super Admin',
      ...
      console.log('✅ Admin user created: admin@shringaar.com / Admin@123');
    }
    ...
    if (serviceCount === 0) {
      const services = [
        ...
        },
        {
          ...
          category: 'wedding',
          icon: '🤵',
          description: 'Beautiful packing for groom\'s suits, sherwani sets, and other formal wear. Custom boxes with elegant finishing touches for gifting at weddings and special occasions.',
    ...
    if (testCount === 0) {
      await Testimonial.insertMany([
        { name: 'Priya Sharma', location: 'Jaipur', message: 'Absolutely loved the lehenga packing! It was so beautifully done, everyone at the wedding was asking about it. Will definitely use again.', rating: 5, eventType: 'wedding', order: 1 },
        { name: 'Ravi Gupta', location: 'Delhi', message: 'Ordered ring ceremony trays and they were beyond expectations. Very professional team and timely delivery. Highly recommended!', rating: 5, eventType: 'wedding', order: 2 },
        { name: 'Meera Agarwal', location: 'Jaipur', message: 'The baby shower packing was so cute and adorable! My guests loved every gift. The personalization made it extra special.', rating: 5, eventType: 'baby-shower', order: 3 },
        { name: 'Sunita Verma', location: 'Bikaner', message: 'Got anniversary gift packing done and it was gorgeous. The rose decoration was fresh and the presentation was perfect. Very happy!', rating: 4, eventType: 'anniversary', order: 4 }
      ]);
